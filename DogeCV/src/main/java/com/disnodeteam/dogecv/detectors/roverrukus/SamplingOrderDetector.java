...
public class SamplingOrderDetector extends DogeCVDetector {
	...
	public Mat process(Mat input) {
		...
		if (chosenYellowRect != null) {
			Imgproc.rectangle(displayMat, new Point(chosenYellowRect.x, chosenYellowRect.y), new Point(chosenYellowRect.x + chosenYellowRect.width, chosenYellowRect.y + chosenYellowRect.height), new Scalar(255, 0, 0), 2);

			Imgproc.putText(displayMat, "Gold: " + String.format("%.2f X=%.2f", chosenYellowScore, (double) chosenYellowRect.x), new Point(chosenYellowRect.x - 5, chosenYellowRect.y - 10), Core.FONT_HERSHEY_PLAIN, 1.3, new Scalar(0, 255, 255), 2);
		...
		for (int i = 0; i < chosenWhiteRect.size(); i++) {
			...
			if (rect != null) {
				...
				Imgproc.rectangle(displayMat, new Point(rect.x, rect.y), new Point(rect.x + rect.width, rect.y + rect.height), new Scalar(255, 255, 255), 2);
				Imgproc.putText(displayMat, "Silver: " + String.format("Score %.2f ", score), new Point(rect.x - 5, rect.y - 10), Core.FONT_HERSHEY_PLAIN, 1.3, new Scalar(255, 255, 255), 2);
			}
		...
		if (chosenWhiteRect.get(0) != null && chosenWhiteRect.get(1) != null && chosenYellowRect != null) {
			...
			int leftIndex = chosenWhiteRect.get(0).x < chosenWhiteRect.get(1).x ? 0 : 1;
			Rect centerRect;

			if (leftCount == 0) {
				currentOrder = SamplingOrderDetector.GoldLocation.LEFT;
				centerRect = chosenWhiteRect.get(1 - leftIndex);
			} else if (leftCount == 1) {
				currentOrder = SamplingOrderDetector.GoldLocation.CENTER;
				centerRect = chosenYellowRect;
			} else {
				currentOrder = SamplingOrderDetector.GoldLocation.RIGHT;
				centerRect = chosenWhiteRect.get(leftIndex);
			}

			center = (centerRect.x + centerRect.width * 0.5) / workingMat.width();
	...
	/**
	 * @return The last known horizontal position of the middle mineral, from 0 (left of screen) to 1 (right of screen)
	 */
